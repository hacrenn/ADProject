ADProject — Análise de Dados (README)
Descrição rápida
ADProject é um projeto de análise de dados que investiga a relação entre indicadores de saúde mental (p.ex. depressão, ansiedade), tempo passado em redes sociais e medidas de bem-estar (p.ex. happiness index). O repositório contém um notebook interativo, um script que automatiza a cadeia de processamento, conjuntos de dados CSV e um relatório com metodologia e conclusões.
Repositório: GitHub — Autor / mantenedor: hacrenn
Tecnologias principais usadas: Python Software Foundation, Project Jupyter
...
Ingestão — carregar CSVs, inspecionar colunas e tipos de dados.
Limpeza — harmonizar nomes de colunas, converter tipos (numérico / data), e lidar com valores em falta (missing values) e outliers.
Harmonização — renomear colunas e mapear variáveis de diferentes fontes para um esquema comum (ex.: Entity → Country).
Merge / Join — combinar datasets por chaves (geralmente Country + Year), escolhendo estratégias de join que preservem observações relevantes.
Transformação — criar variáveis derivadas (normalizações, percentis, categorizações por faixa etária).
Exploração — estatísticas descritivas, correlações, pivot tables e visualizações (mapas coropléticos, séries temporais, scatter plots estratificados).
Interpretação — sumarizar padrões observados, limitações e hipóteses para investigação adicional.
